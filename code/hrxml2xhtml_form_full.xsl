...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml" xmlns:hr="http://ns.hr-xml.org/2006-02-28" xmlns:xi="http://www.w3.org/2001/XInclude" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:iso="http://www.iso.org/" xmlns:exslt="http://exslt.org/common" version="1.0" exclude-result-prefixes="xsl hr xi xsd #default iso exslt">
<!-- {{{ COPYING 
...
Foundation, Inc., 59 Temple Place, Suite 330, Boston, MA  02111-1307  USA
    ...
    <div id="EuropassCV">
     ...
     <!-- {{{ Section: intro -->
      ...
      <div id="EuropassCV_Header">
       <a href="http://europass.cedefop.europa.eu/" target="_blank" title="Europass web-site">
       <img src="_htReqsURL_/icons/europass.png" alt="Europass"/></a>
      </div>
      <div class="EuropassCV_ContentLine">
       <div id="EuropassCV_Title" class="EuropassCV_HeaderColumn">
        __(Europass Curriculum Vitae)
       </div>
       <div class="EuropassCV_Content">
        <a href="http://europass.cedefop.eu.int/europass/home/hornav/Downloads/EuropassCV/CVInstructions/navigate.action" target="_blank">__(Click here to download instructions for filling in this form)</a>
       </div>
      </div>
      <input type="image" onClick="javascript:submitFormSettingNextPage('hrxml2xhtml_photo.xsl',this);" style="position: absolute; right: 0; top: 0; background-color: #eee; border: outset thin black; width: 100px; height: 100px; text-align:center;" src="_exportURL_?position=0&amp;format=photo">
       <xsl:attribute name="alt">__(Upload a photo)</xsl:attribute>
      </input>

     <!-- }}}  -->
     <!-- {{{ Section: personal infos -->

     <div class="EuropassCV_ContentLineNewSection multi-part" title="__(Personal)">
      ...
      <div class="EuropassCV_ContentLine">
       ...
       <div class="EuropassCV_Content">
        <input type="text" name="/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:ContactInfo/hr:PersonName/hr:FormattedName" class="wide">
         <xsl:attribute name="value">
          <xsl:value-of select="/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:ContactInfo/hr:PersonName/hr:FormattedName"/>
         </xsl:attribute>
        </input>
       </div>
       <div class="KiteCV_help">
        __(If you have more than one surname, start with the one you usually use.)
       </div>
      </div>
      ...
      <div class="EuropassCV_ContentLine">
       ...
       <div class="EuropassCV_Content">
        <xsl:for-each select="/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:ContactInfo/hr:ContactMethod/hr:PostalAddress">
         <xsl:variable name="postalAddressPath">/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:ContactInfo/hr:ContactMethod/hr:PostalAddress(<xsl:number/>)</xsl:variable>
         <div class="KiteCV_deletable">
          <xsl:attribute name="title"><xsl:value-of select="$postalAddressPath"/></xsl:attribute>
          <xsl:call-template name="postalAddress">
           ...
           <xsl:with-param name="tabIndex">0</xsl:with-param>
          </xsl:call-template>
         </div>
        </xsl:for-each>
        <div class="KiteCV_adder">
         <button type="button">__(Add an address)</button>
         <div class="KiteCV_adder_content">
          <xsl:call-template name="postalAddress">
           <xsl:with-param name="path">/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:ContactInfo/hr:ContactMethod/hr:PostalAddress(<xsl:value-of select="count(/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:ContactInfo/hr:ContactMethod/hr:PostalAddress) + 1"/>)</xsl:with-param>
           <xsl:with-param name="node"><xsl:copy-of select="/doesnotexist"/></xsl:with-param>
           <xsl:with-param name="tabIndex">0</xsl:with-param>
          </xsl:call-template>
         </div>
        </div>
      ...
      <div class="EuropassCV_ContentLine">
       <div class="EuropassCV_HeaderColumn">
        __(Telephone(s) - see) <a href="http://publications.eu.int/code/en/en-390200.htm" target="_blank">__(the interinstitutional style guide)</a>
       </div>
       <div class="EuropassCV_Content">
        <xsl:for-each select="/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:ContactInfo/hr:ContactMethod[hr:Telephone]">
         <xsl:variable name="contactMethodPath">/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:ContactInfo/hr:ContactMethod(hr:Telephone)(<xsl:number count="hr:ContactMethod[hr:Telephone]"/>)</xsl:variable>
         <div class="KiteCV_deletable">
          <xsl:attribute name="title"><xsl:value-of select="$contactMethodPath"/></xsl:attribute>
        ...
        <div class="KiteCV_adder">
         <button type="button">__(Add a telephone number)</button>
         <div class="KiteCV_adder_content">
          <input type="text" class="wide">
           <xsl:attribute name="name">/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:ContactInfo/hr:ContactMethod(<xsl:value-of select="count(/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:ContactInfo/hr:ContactMethod) + 1"/>)/hr:Telephone/hr:FormattedNumber</xsl:attribute>
          </input>
         </div>
        </div>
        <div class="KiteCV_help">
         __(State the telephone number(s) where you wish to be contacted;&lt;br /&gt; if necessary, give specific days and times when it is possible to reach you (so that you can be contacted quickly), e.g.: &lt;blockquote&gt; Fixed: 0845 120 30 45 &lt;/blockquote&gt;)
        </div>
       </div>
      </div>
      ...
      <div class="EuropassCV_ContentLine">
       ...
       <div class="EuropassCV_Content">
        <xsl:for-each select="/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:ContactInfo/hr:ContactMethod[hr:Fax]">
         <xsl:variable name="contactMethodPath">/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:ContactInfo/hr:ContactMethod(hr:Fax)(<xsl:number count="hr:ContactMethod[hr:Fax]"/>)</xsl:variable>
         <div class="KiteCV_deletable">
          <xsl:attribute name="title"><xsl:value-of select="$contactMethodPath"/></xsl:attribute>
          <input type="text" class="wide">
        ...
        <div class="KiteCV_adder">
         <button type="button">__(Add a fax number)</button>
         <div class="KiteCV_adder_content">
          <input type="text" class="wide">
           <xsl:attribute name="name">/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:ContactInfo/hr:ContactMethod(<xsl:value-of select="count(/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:ContactInfo/hr:ContactMethod) + 1"/>)/hr:Fax/hr:FormattedNumber</xsl:attribute>
          </input>
         </div>
        </div>
       </div>
      ...
      <div class="EuropassCV_ContentLine">
       ...
       <div class="EuropassCV_Content">
        <xsl:for-each select="/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:ContactInfo/hr:ContactMethod[hr:InternetEmailAddress]">
         <xsl:variable name="contactMethodPath">/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:ContactInfo/hr:ContactMethod(hr:InternetEmailAddress)(<xsl:number count="hr:ContactMethod[hr:InternetEmailAddress]"/>)</xsl:variable>
         <div class="KiteCV_deletable">
          <xsl:attribute name="title"><xsl:value-of select="$contactMethodPath"/></xsl:attribute>
          <input type="text" class="wide">
        ...
        <div class="KiteCV_adder">
         <button type="button">__(Add an email address)</button>
         <div class="KiteCV_adder_content">
          <input type="text" class="wide">
           <xsl:attribute name="name">/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:ContactInfo/hr:ContactMethod(<xsl:value-of select="count(/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:ContactInfo/hr:ContactMethod) + 1"/>)/hr:InternetEmailAddress</xsl:attribute>
          </input>
         </div>
        </div>
       </div>
      ...
      <div class="EuropassCV_ContentLine">
       ...
       <div class="EuropassCV_Content">
        ...
        <xsl:for-each select="/hr:Candidate/hr:Resume/hr:UserArea/hr:Nationality">
         <xsl:variable name="nationalityPath">/hr:Candidate/hr:Resume/hr:UserArea/hr:Nationality(<xsl:number/>)</xsl:variable>
         <div class="KiteCV_deletable">
          <xsl:attribute name="title"><xsl:value-of select="$nationalityPath"/></xsl:attribute>
          <input type="text" class="wide">
        ...
        <div class="KiteCV_adder">
         <button type="button">__(Add a nationality)</button>
         <div class="KiteCV_adder_content">
          <input type="text" class="wide">
           <xsl:attribute name="name">/hr:Candidate/hr:Resume/hr:UserArea/hr:Nationality(<xsl:value-of select="count(/hr:Candidate/hr:Resume/hr:UserArea/hr:Nationality) + 1"/>)</xsl:attribute>
          </input>
         </div>
        </div>
       </div>
      ...
      <div class="EuropassCV_ContentLine">
       ...
       <div class="EuropassCV_Content">
        <xsl:call-template name="date">
         <xsl:with-param name="path">/hr:Candidate/hr:Resume/hr:UserArea/hr:DateOfBirth/hr:AnyDate</xsl:with-param>
         <xsl:with-param name="value"><xsl:value-of select="/hr:Candidate/hr:Resume/hr:UserArea/hr:DateOfBirth/hr:AnyDate"/></xsl:with-param>
        </xsl:call-template>
       </div>
      </div>
      ...
      <div class="EuropassCV_ContentLine">
       ...
       <div class="EuropassCV_Content">
        <input type="radio" name="/hr:Candidate/hr:Resume/hr:UserArea/hr:Gender" value="1">
         <xsl:if test="/hr:Candidate/hr:Resume/hr:UserArea/hr:Gender='1'">
          <xsl:attribute name="checked">1</xsl:attribute>
         </xsl:if>
         __(M)
        </input>
        <input type="radio" name="/hr:Candidate/hr:Resume/hr:UserArea/hr:Gender" value="2">
         <xsl:if test="/hr:Candidate/hr:Resume/hr:UserArea/hr:Gender='2'">
          <xsl:attribute name="checked">1</xsl:attribute>
         </xsl:if>
         __(F)
        </input>
        <input type="radio" name="/hr:Candidate/hr:Resume/hr:UserArea/hr:Gender" value="1">
         <xsl:if test="/hr:Candidate/hr:Resume/hr:UserArea/hr:Gender='9'">
          <xsl:attribute name="checked">1</xsl:attribute>
         </xsl:if>
         __(Not specified)
        </input>
      ...
      <div class="EuropassCV_ContentLine">
       ...
       <div class="EuropassCV_Content">
        <input type="text" name="/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:Objective" class="wide">
         <xsl:attribute name="value">
          <xsl:value-of select="/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:Objective"/>
         </xsl:attribute>
        </input>
        <div class="KiteCV_help">
         __(State your main activities and responsibilities, e.g.: &lt;blockquote&gt; Maintenance of computers &lt;blockquote&gt; &lt;b&gt;or&lt;b&gt; &lt;blockquote&gt; Relations with suppliers &lt;blockquote&gt; &lt;b&gt;or&lt;b&gt; &lt;blockquote&gt; Maintaining parks and gardens &lt;blockquote&gt;)
        </div>
       </div>
      </div>
     ...
     <div class="EuropassCV_ContentLineNewSection multi-part" title="__(Work)">
      <div class="EuropassCV_HeaderColumn EuropassCV_SectionTitle">
      ...
      <xsl:for-each select="/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:EmploymentHistory/hr:EmployerOrg">
       <xsl:variable name="employerOrgPath">/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:EmploymentHistory/hr:EmployerOrg(<xsl:number/>)</xsl:variable>
       <div class="KiteCV_deletable">
        <xsl:attribute name="title"><xsl:value-of select="$employerOrgPath"/></xsl:attribute>
       ...
       <div class="EuropassCV_ContentLine">
        ...
        <div class="EuropassCV_Content">
         ...
         <br/>
         <xsl:call-template name="date">
          <xsl:with-param name="path"><xsl:value-of select="$employerOrgPath"/>/hr:PositionHistory(1)/hr:StartDate/hr:AnyDate</xsl:with-param>
          <xsl:with-param name="value"><xsl:value-of select="hr:PositionHistory[1]/hr:StartDate/hr:AnyDate"/></xsl:with-param>
         </xsl:call-template>
         <br/>
         <label>__(End date)</label>
         <br/>
         <xsl:call-template name="date">
          <xsl:with-param name="path"><xsl:value-of select="$employerOrgPath"/>/hr:PositionHistory(1)/hr:EndDate/hr:AnyDate</xsl:with-param>
          <xsl:with-param name="value"><xsl:value-of select="hr:PositionHistory[1]/hr:EndDate/hr:AnyDate"/></xsl:with-param>
         </xsl:call-template>
        </div>
       ...
       <div class="EuropassCV_ContentLine">
        <div class="EuropassCV_HeaderColumn">
         __(Occupation or position held)
        </div>
        <div class="EuropassCV_Content">
         <input type="text" class="wide">
          <xsl:attribute name="name"><xsl:value-of select="$employerOrgPath"/>/hr:PositionHistory/hr:Title</xsl:attribute>
          <xsl:attribute name="value"><xsl:value-of select="hr:PositionHistory/hr:Title"/></xsl:attribute>
         </input>
        </div>
       </div>
       ...
       <div class="EuropassCV_ContentLine">
        <div class="EuropassCV_HeaderColumn">
         __(Main activities or responsabilities)
        </div>
        <div class="EuropassCV_Content">
         <textarea type="text" class="wide">
          <xsl:attribute name="name"><xsl:value-of select="$employerOrgPath"/>/hr:PositionHistory/hr:Description</xsl:attribute>
          <xsl:value-of select="hr:PositionHistory/hr:Description"/>
          <xsl:text> </xsl:text>
         </textarea>
        </div>
       </div>
       ...
       <div class="EuropassCV_ContentLine">
        <div class="EuropassCV_HeaderColumn">
         __(Name and address of employer)
        </div>
        <div class="EuropassCV_Content">
         <label>__(Organisation name)</label>
         <br/>
         <input type="hidden">
          <xsl:attribute name="id"><xsl:value-of select="$employerOrgPath"/>/hr:PositionHistory(1)/hr:OrgName/hr:OrganizationName</xsl:attribute>
          <xsl:attribute name="name"><xsl:value-of select="$employerOrgPath"/>/hr:PositionHistory(1)/hr:OrgName/hr:OrganizationName</xsl:attribute>
          <xsl:attribute name="value"><xsl:value-of select="hr:PositionHistory[1]/hr:OrgName/hr:OrganizationName"/></xsl:attribute>
         </input>
         <input type="text" class="wide">
          <xsl:attribute name="onChange">javascript:getElementById('<xsl:value-of select="$employerOrgPath"/>/hr:PositionHistory(1)/hr:OrgName/hr:OrganizationName').value=this.value;</xsl:attribute>
          <xsl:attribute name="name"><xsl:value-of select="$employerOrgPath"/>/hr:EmployerOrgName</xsl:attribute>
          <xsl:attribute name="value"><xsl:value-of select="hr:EmployerOrgName"/></xsl:attribute>
         </input>
         <br/>
         <label>__(Organisation address)</label>
         <xsl:call-template name="postalAddress">
          <xsl:with-param name="path"><xsl:value-of select="$employerOrgPath"/>/hr:EmployerContactInfo/hr:ContactMethod/hr:PostalAddress</xsl:with-param>
          <xsl:with-param name="node"><xsl:copy-of select="hr:EmployerContactInfo/hr:ContactMethod/hr:PostalAddress"/></xsl:with-param>
          <xsl:with-param name="tabIndex">0</xsl:with-param>
         </xsl:call-template>
        </div>
       </div>
       ...
       <div class="EuropassCV_ContentLine">
        <div class="EuropassCV_HeaderColumn">
         __(Type of business or sector)
        </div>
        <div class="EuropassCV_Content">
         <input type="text" class="wide">
          <xsl:attribute name="name"><xsl:value-of select="$employerOrgPath"/>/hr:PositionHistory(1)/hr:OrgIndustry(1)/hr:IndustryDescription</xsl:attribute>
          <xsl:attribute name="value"><xsl:value-of select="hr:PositionHistory[1]/hr:OrgIndustry[1]/hr:IndustryDescription"/></xsl:attribute>
         </input>
        </div>
       </div>
      ...
      <div class="KiteCV_adder">
       <button type="button">__(Add a work experience)</button>
    <div class="KiteCV_adder_content">
        <!-- {{{ dates -->
        <div class="EuropassCV_ContentLine">
         <div class="EuropassCV_HeaderColumn">
         ...
         <div class="EuropassCV_Content">
          ...
          <br/>
          <xsl:call-template name="date">
           <xsl:with-param name="path"><xsl:value-of select="$newEmployerOrgPath"/>/hr:PositionHistory(1)/hr:StartDate/hr:AnyDate</xsl:with-param>
          </xsl:call-template>
          <br/>
          <label>__(End date)</label>
          <br/>
          <xsl:call-template name="date">
           <xsl:with-param name="path"><xsl:value-of select="$newEmployerOrgPath"/>/hr:PositionHistory(1)/hr:EndDate/hr:AnyDate</xsl:with-param>
          </xsl:call-template>
         </div>
        </div>
        <!-- }}} -->
        ...
        <div class="EuropassCV_ContentLine">
         <div class="EuropassCV_HeaderColumn">
          __(Occupation of position held)
         </div>
         <div class="EuropassCV_Content">
          <input type="text" class="wide">
           <xsl:attribute name="name"><xsl:value-of select="$newEmployerOrgPath"/>/hr:PositionHistory/hr:Title</xsl:attribute>
          </input>
         </div>
        </div>
        <!-- }}} -->
        ...
        <div class="EuropassCV_ContentLine">
         ...
         <div class="EuropassCV_Content">
          <textarea type="text" class="wide">
           <xsl:attribute name="name"><xsl:value-of select="$newEmployerOrgPath"/>/hr:PositionHistory/hr:Description</xsl:attribute>
           <xsl:text> </xsl:text>
          </textarea>
         </div>
        </div>
        ...
        <div class="EuropassCV_ContentLine">
         ...
         <div class="EuropassCV_Content">
          ...
          <br/>
          <input type="hidden">
           <xsl:attribute name="id"><xsl:value-of select="$newEmployerOrgPath"/>/hr:PositionHistory(1)/hr:OrgName/hr:OrganizationName</xsl:attribute>
           <xsl:attribute name="name"><xsl:value-of select="$newEmployerOrgPath"/>/hr:PositionHistory(1)/hr:OrgName/hr:OrganizationName</xsl:attribute>
          </input>
          <input type="text" class="wide">
           <xsl:attribute name="onChange">javascript:getElementById('<xsl:value-of select="$newEmployerOrgPath"/>/hr:PositionHistory(1)/hr:OrgName/hr:OrganizationName').value=this.value;</xsl:attribute>
           <xsl:attribute name="name"><xsl:value-of select="$newEmployerOrgPath"/>/hr:EmployerOrgName</xsl:attribute>
          </input>
          <br/>
          <xsl:call-template name="postalAddress">
           <xsl:with-param name="path"><xsl:value-of select="$newEmployerOrgPath"/>/hr:EmployerContactInfo/hr:ContactMethod/hr:PostalAddress</xsl:with-param>
           <xsl:with-param name="node"><xsl:copy-of select="/doesnotExist"/></xsl:with-param>
        ...
        <div class="EuropassCV_ContentLine">
         ...
         <div class="EuropassCV_Content">
          <input type="text" class="wide">
           <xsl:attribute name="name"><xsl:value-of select="$newEmployerOrgPath"/>/hr:PositionHistory(1)/hr:OrgIndustry(1)/hr:IndustryDescription</xsl:attribute>
          </input>
         </div>
        </div>
        <!-- }}} -->
     ...
     <div class="EuropassCV_ContentLineNewSection multi-part" title="__(Education)">
      <div class="EuropassCV_HeaderColumn EuropassCV_SectionTitle">
       <span id="EducationAndTraining">__(Education and training)</span>
      </div>
 
      <xsl:for-each select="/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:EducationHistory/hr:SchoolOrInstitution">
       <xsl:variable name="schoolOrInstitutionPath">/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:EducationHistory/hr:SchoolOrInstitution(<xsl:number/>)</xsl:variable>
       <xsl:for-each select="hr:Degree">
        <xsl:variable name="degreePath"><xsl:value-of select="$schoolOrInstitutionPath"/>/hr:Degree(<xsl:number/>)</xsl:variable>
        <div class="KiteCV_deletable">
         <xsl:attribute name="title"><xsl:value-of select="$degreePath"/></xsl:attribute>
        ...
        <div class="EuropassCV_ContentLine">
         ...
         <div class="EuropassCV_Content">
          ...
          <br/>
          <xsl:call-template name="date">
           <xsl:with-param name="path"><xsl:value-of select="$degreePath"/>/hr:DatesOfAttendance(1)/hr:StartDate/hr:AnyDate</xsl:with-param>
           <xsl:with-param name="value"><xsl:value-of select="hr:DatesOfAttendance[1]/hr:StartDate/hr:AnyDate"/></xsl:with-param>
          </xsl:call-template>
          <br/>
          <label>__(End date)</label>
          <br/>
          <xsl:call-template name="date">
           <xsl:with-param name="path"><xsl:value-of select="$degreePath"/>/hr:DatesOfAttendance(1)/hr:EndDate/hr:AnyDate</xsl:with-param>
           <xsl:with-param name="value"><xsl:value-of select="hr:DatesOfAttendance[1]/hr:EndDate/hr:AnyDate"/></xsl:with-param>
          </xsl:call-template>
         </div>
        </div>
        <div class="EuropassCV_ContentLine">
         <div class="EuropassCV_HeaderColumn">
          __(Title of qualification awarded)
         </div>
         <div class="EuropassCV_Content">
          <input type="text" class="wide">
           <xsl:attribute name="name"><xsl:value-of select="$degreePath"/>/hr:DegreeName</xsl:attribute>
           <xsl:attribute name="value"><xsl:value-of select="hr:DegreeName"/></xsl:attribute>
          </input>
         </div>
        </div>
        <div class="EuropassCV_ContentLine">
         <div class="EuropassCV_HeaderColumn">
          __(Principal subjects/occupational skills covered)
         </div>
         <div class="EuropassCV_Content">
          <textarea type="text" class="wide">
           <xsl:attribute name="name"><xsl:value-of select="$degreePath"/>/hr:Comments</xsl:attribute>
           <xsl:value-of select="hr:Comments"/>
           <xsl:text> </xsl:text>
          </textarea>
         </div>
        </div>
        <div class="EuropassCV_ContentLine">
         <div class="EuropassCV_HeaderColumn">
          __(Name and type of organization providing education and training)
         </div>
         <div class="EuropassCV_Content">
          <input type="text" class="wide">
           <xsl:attribute name="name"><xsl:value-of select="$schoolOrInstitutionPath"/>/hr:School/hr:SchoolName</xsl:attribute>
           <xsl:attribute name="value"><xsl:value-of select="../hr:School/hr:SchoolName"/></xsl:attribute>
          </input>
         </div>
        </div>
        <div class="EuropassCV_ContentLine">
         <div class="EuropassCV_HeaderColumn">
          __(Level in national or international classification (optional))
         </div>
         <div class="EuropassCV_Content">
          <label>__(Level name, code or number)</label>
          <br/>
          <input type="text" class="wide">
           <xsl:attribute name="name"><xsl:value-of select="$degreePath"/>/hr:UserArea/hr:ReferenceLevel</xsl:attribute>
           <xsl:attribute name="value"><xsl:value-of select="hr:UserArea/hr:ReferenceLevel"/></xsl:attribute>
          </input>
          <br/>
          <label>__(Reference classification system name)</label>
          <input type="text" class="wide">
           <xsl:attribute name="name"><xsl:value-of select="$degreePath"/>/hr:UserArea/hr:ReferenceLevel/@referenceType</xsl:attribute>
           <xsl:attribute name="value"><xsl:value-of select="hr:UserArea/hr:ReferenceLevel/@referenceType"/></xsl:attribute>
          </input>
         </div>
        </div>
        </div>
       </xsl:for-each> <!-- Degree -->
      </xsl:for-each> <!-- SchoolOrInstitution -->
      <div class="KiteCV_adder">
       <button type="button">__(Add a degree)</button>
       <div class="KiteCV_adder_content">
        <xsl:variable name="newSchoolOrInstitutionPath">/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:EducationHistory/hr:SchoolOrInstitution(<xsl:value-of select="count(/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:EducationHistory/hr:SchoolOrInstitution) + 1"/>)</xsl:variable>
        <xsl:variable name="newDegreePath"><xsl:value-of select="$newSchoolOrInstitutionPath"/>/hr:Degree(1)</xsl:variable>
        <div class="EuropassCV_ContentLine">
         <div class="EuropassCV_HeaderColumn">
          __(Dates)
         </div>
         <div class="EuropassCV_Content">
          ...
          <br/>
          <xsl:call-template name="date">
           <xsl:with-param name="path"><xsl:value-of select="$newDegreePath"/>/hr:DatesOfAttendance(1)/hr:StartDate/hr:AnyDate</xsl:with-param>
          </xsl:call-template>
          <br/>
          <label>__(End date)</label>
          <br/>
          <xsl:call-template name="date">
           <xsl:with-param name="path"><xsl:value-of select="$newDegreePath"/>/hr:DatesOfAttendance(1)/hr:EndDate/hr:AnyDate</xsl:with-param>
          </xsl:call-template>
         </div>
        </div>
        <div class="EuropassCV_ContentLine">
         ...
         <div class="EuropassCV_Content">
          <input type="text" class="wide">
           <xsl:attribute name="name"><xsl:value-of select="$newDegreePath"/>/hr:DegreeName</xsl:attribute>
          </input>
         </div>
        </div>
        <div class="EuropassCV_ContentLine">
         ...
         <div class="EuropassCV_Content">
          <textarea type="text" class="wide">
           <xsl:attribute name="name"><xsl:value-of select="$newDegreePath"/>/hr:Comments</xsl:attribute>
           <xsl:text> </xsl:text>
          </textarea>
         </div>
        </div>
        <div class="EuropassCV_ContentLine">
         ...
         <div class="EuropassCV_Content">
          <input type="text" class="wide">
           <xsl:attribute name="name"><xsl:value-of select="$newSchoolOrInstitutionPath"/>/hr:School/hr:SchoolName</xsl:attribute>
          </input>
         </div>
        </div>
        <div class="EuropassCV_ContentLine">
         ...
         <div class="EuropassCV_Content">
          ...
          <br/>
          <input type="text" class="wide">
           <xsl:attribute name="name"><xsl:value-of select="$newDegreePath"/>/hr:UserArea/hr:ReferenceLevel</xsl:attribute>
          </input>
          <br/>
          <label>__(Reference classification system name)</label>
          <br/>
          <input type="text" class="wide">
           <xsl:attribute name="name"><xsl:value-of select="$newDegreePath"/>/hr:UserArea/hr:ReferenceLevel/@referenceType</xsl:attribute>
          </input>
         </div>
        </div>
       </div>
     ...
     <div class="EuropassCV_ContentLineNewSection multi-part" title="__(Languages)">
      <div class="EuropassCV_HeaderColumn EuropassCV_SectionTitle">
       <span id="Language">__(Personal skills and competences (language part))</span>
      </div>
      ...
      <div class="EuropassCV_ContentLine">
       ...
       <div class="EuropassCV_Content">
        <xsl:for-each select="/hr:Candidate/hr:Resume/hr:UserArea/hr:MotherLanguages/hr:MotherLanguage">
         <xsl:variable name="motherLanguagePath">/hr:Candidate/hr:Resume/hr:UserArea/hr:MotherLanguages/hr:MotherLanguage(<xsl:number/>)</xsl:variable>
         <div class="KiteCV_deletable">
        ...
        <div class="KiteCV_adder">
         <button type="button">__(Add a mother tongue)</button>
         <div class="KiteCV_adder_content">
          <select>
           <xsl:attribute name="name">/hr:Candidate/hr:Resume/hr:UserArea/hr:MotherLanguages/hr:MotherLanguage(<xsl:value-of select="count(/hr:Candidate/hr:Resume/hr:UserArea/hr:MotherLanguages/hr:MotherLanguage) + 1"/>)</xsl:attribute>
           <xsl:call-template name="languageSelectOptions">
            <xsl:with-param name="code"/>
           </xsl:call-template>
          </select>
       ...
       <div class="EuropassCV_Content">
        <table>
         <tr>
          ...
          <th rowspan="2" width="20%">__(Writing)</th>
         </tr>
         <tr>
          <th width="20%">__(Listening)</th>
          <th width="20%">__(Reading)</th>
          <th width="20%">__(Spoken interaction)</th>
          <th width="20%">__(Spoken production)</th>
         </tr>
        </table>
       </div>
      </div>
      <xsl:for-each select="/hr:Candidate/hr:Resume/hr:UserArea/hr:OtherLanguages/hr:OtherLanguage">
       <xsl:variable name="otherLanguagePath">/hr:Candidate/hr:Resume/hr:UserArea/hr:OtherLanguages/hr:OtherLanguage(<xsl:number/>)</xsl:variable>
       <div class="KiteCV_deletable">
        <xsl:attribute name="title"><xsl:value-of select="$otherLanguagePath"/></xsl:attribute>
       <div class="EuropassCV_ContentLine">
        <div class="EuropassCV_HeaderColumn">
         <select>
      ...
      <div class="KiteCV_adder">
       <button type="button">__(Add another language)</button>
       <div class="KiteCV_adder_content">
        <xsl:variable name="newOtherLanguagePath">/hr:Candidate/hr:Resume/hr:UserArea/hr:OtherLanguages/hr:OtherLanguage(<xsl:value-of select="count(/hr:Candidate/hr:Resume/hr:UserArea/hr:OtherLanguages/hr:OtherLanguage) + 1"/>)</xsl:variable>
        <div class="EuropassCV_HeaderColumn">
         <select>
          <xsl:attribute name="name"><xsl:value-of select="$newOtherLanguagePath"/>/hr:CompetencyId/@id</xsl:attribute>
          <xsl:call-template name="languageSelectOptions">
           <xsl:with-param name="code"/>
          </xsl:call-template>
         </select>
        ...
        <div class="EuropassCV_Content">
         <table>
          <tr>
           <td width="20%">
            <select>
             <xsl:attribute name="name"><xsl:value-of select="$newOtherLanguagePath"/>/hr:CompetencyEvidence(@id='listen')/hr:StringValue</xsl:attribute>
             <xsl:call-template name="ELPSelectOptions"/>
            </select>
           </td>
           <td width="20%">
            <select>
             <xsl:attribute name="name"><xsl:value-of select="$newOtherLanguagePath"/>/hr:CompetencyEvidence(@id='read')/hr:StringValue</xsl:attribute>
             <xsl:call-template name="ELPSelectOptions"/>
            </select>
           </td>
           <td width="20%">
            <select>
             <xsl:attribute name="name"><xsl:value-of select="$newOtherLanguagePath"/>/hr:CompetencyEvidence(@id='talk')/hr:StringValue</xsl:attribute>
             <xsl:call-template name="ELPSelectOptions"/>
            </select>
           </td>
           <td width="20%">
            <select>
             <xsl:attribute name="name"><xsl:value-of select="$newOtherLanguagePath"/>/hr:CompetencyEvidence(@id='speech')/hr:StringValue</xsl:attribute>
             <xsl:call-template name="ELPSelectOptions"/>
            </select>
           </td>
           <td width="20%">
            <select>
             <xsl:attribute name="name"><xsl:value-of select="$newOtherLanguagePath"/>/hr:CompetencyEvidence(@id='write')/hr:StringValue</xsl:attribute>
             <xsl:call-template name="ELPSelectOptions"/>
            </select>
           </td>
          </tr>
     ...
     <div class="EuropassCV_ContentLineNewSection multi-part" title="__(Other competences)">
      <div class="EuropassCV_HeaderColumn EuropassCV_SectionTitle">
     ...
     <div class="EuropassCV_ContentLineNewSection multi-part" title="__(Additional Information)">
      <div class="EuropassCV_HeaderColumn EuropassCV_SectionTitle">
       <span id="AdditionalInformation">__(Additional Information)</span>
      </div>
      <div class="EuropassCV_ContentLine">
       <div class="EuropassCV_HeaderColumn">
        <br/>
       ...
       <div class="EuropassCV_Content">
        <xsl:for-each select="/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:ResumeAdditionalItems/hr:ResumeAdditionalItem">
         <xsl:variable name="resumeAdditionalItemPath">/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:ResumeAdditionalItems/hr:ResumeAdditionalItem(<xsl:number/>)</xsl:variable>
         <div class="KiteCV_deletable">
          <xsl:attribute name="title"><xsl:value-of select="$resumeAdditionalItemPath"/></xsl:attribute>
         <textarea class="wide">
        ...
        <div class="KiteCV_adder">
         <button type="button">__(Add an information)</button>
         <div class="KiteCV_adder_content">
          <textarea class="wide">
           <xsl:attribute name="name">/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:ResumeAdditionalItems/hr:ResumeAdditionalItem(<xsl:value-of select="count(/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:ResumeAdditionalItems/hr:ResumeAdditionalItem) + 1"/>)/hr:Description</xsl:attribute>
           <xsl:text> </xsl:text>
          </textarea>
         </div>
        </div>
     ...
     <div class="EuropassCV_ContentLineNewSection multi-part" title="__(Annexes)">
      <div class="EuropassCV_HeaderColumn EuropassCV_SectionTitle">
       <span id="annexes">__(Annexes)</span>
      </div>
      <div class="EuropassCV_ContentLine">
       <div class="EuropassCV_HeaderColumn">
        <br/>
       ...
       <div class="EuropassCV_Content">
        <xsl:for-each select="/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:SupportingMaterials">
         <xsl:variable name="supportingMaterialsPath">/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:SupportingMaterials(<xsl:number/>)</xsl:variable>
         <div class="KiteCV_deletable">
          <xsl:attribute name="title"><xsl:value-of select="$supportingMaterialsPath"/></xsl:attribute>
        ...
        <div class="KiteCV_adder">
         <button type="button">__(Add an annexe)</button>
         <div class="KiteCV_adder_content">
          <xsl:variable name="newSupportingMaterialsPath">/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:SupportingMaterials(<xsl:value-of select="count(/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:SupportingMaterials) + 1"/>)</xsl:variable>
          <label>__(Description)</label>
          <br/>
          <input type="text" class="wide">
           <xsl:attribute name="name"><xsl:value-of select="$newSupportingMaterialsPath"/>/hr:Description</xsl:attribute>
          </input>
          <br/>
          <label>__(Link (URL))</label>
          <br/>
          <input type="text" class="wide">
           <xsl:attribute name="name"><xsl:value-of select="$newSupportingMaterialsPath"/>/hr:Link</xsl:attribute>
          </input>
          <br/>
         </div>
        </div>
     ...
     <!-- }}} -->
     <div class="EuropassCV_ContentLine">
      <button type="submit" class="KiteCV_submit" onclick="KiteCVSaveSubmitForm(this); return false;">Save this CV</button>
     </div>
    </div> <!-- id=EuropassCV -->
    <script type="text/javascript">
     kiteCVSetTabIndex(<xsl:value-of select="$tabIndex"/>);
    </script>
 </xsl:template>

 <!-- {{{ Template: competencySection -->
  ...
  <div class="EuropassCV_ContentLine">
   ...
   <div class="EuropassCV_Content">
    <xsl:for-each select="/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:Qualifications/hr:Competency[@name=$name]">
     <xsl:variable name="competencyPath">/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:Qualifications/hr:Competency(@name='<xsl:value-of select="$name"/>')(<xsl:number count="hr:Competency[@name=$name]"/>)</xsl:variable>
     <div class="KiteCV_deletable">
      <xsl:attribute name="title"><xsl:value-of select="$competencyPath"/></xsl:attribute>
    ...
    <div class="KiteCV_adder">
     <button type="button">__(Add a competency)</button>
     <div class="KiteCV_adder_content">
      <xsl:attribute name="id">new<xsl:value-of select="$name"/>Competency</xsl:attribute>
      <xsl:call-template name="competencyTextarea">
       <xsl:with-param name="path">/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:Qualifications/hr:Competency(@name='<xsl:value-of select="$name"/>')(<xsl:value-of select="count(/hr:Candidate/hr:Resume/hr:StructuredXMLResume/hr:Qualifications/hr:Competency[@name=$name]) + 1"/>)</xsl:with-param>
 ...
 <!-- }}} -->
 <!-- {{{ Template: competencyTextarea -->

 <xsl:template name="competencyTextarea">
  <xsl:param name="path"/>
  <xsl:param name="value"/>
  <textarea class="wide">
   <xsl:attribute name="name"><xsl:value-of select="$path"/>/@description</xsl:attribute>
 ...
 <!-- }}} -->
 <!-- {{{ Template: languageSelectOptions -->
 ...
 <xsl:template name="date">
  ...
  <input type="text">
   ...
   <xsl:attribute name="id">input_<xsl:value-of select="$path"/></xsl:attribute>
   <xsl:attribute name="value"><xsl:value-of select="$value"/></xsl:attribute>
  </input>
  <button type="reset">
   <xsl:attribute name="id">button_<xsl:value-of select="$path"/></xsl:attribute>
   ...
  </button>
  <script type="text/javascript">
   Calendar.setup({
    inputField : "input_<xsl:value-of select="$path"/>",
    ifFormat   : "%Y-%m-%d",
    button     : "button_<xsl:value-of select="$path"/>"
   });
  </script>
 </xsl:template>
 ...
 <xsl:template name="postalAddress">
  ...
  <xsl:for-each select="exslt:node-set($node)/hr:PostalAddress/hr:DeliveryAddress/hr:AddressLine">
   ...
   <div class="KiteCV_deletable">
    <xsl:attribute name="title"><xsl:value-of select="$addressLinePath"/></xsl:attribute>
   <input type="text" class="wide">
    <xsl:attribute name="name"><xsl:value-of select="$path"/><xsl:value-of select="$addressLinePath"/></xsl:attribute>
    <xsl:attribute name="value"><xsl:value-of select="."/></xsl:attribute>
   </input>
   </div>
  </xsl:for-each>
  <div class="KiteCV_adder">
   <button type="button">__(Add a line)</button>
   <div class="KiteCV_adder_content">
    <input type="text" class="wide">
  ...
  <br/>
  <input type="text" class="wide">
   <xsl:attribute name="name"><xsl:value-of select="$path"/>/hr:Municipality</xsl:attribute>
   <xsl:attribute name="value"><xsl:value-of select="exslt:node-set($node)/hr:PostalAddress/hr:Municipality"/></xsl:attribute>
  </input>
  <br/>
  <label>__(Postal Code - see) <a href="http://publications.eu.int/code/en/en-370103.htm">__(the interinstitutional style guide)</a></label>
  <br/>
  <input type="text">
   <xsl:attribute name="name"><xsl:value-of select="$path"/>/hr:PostalCode</xsl:attribute>
   <xsl:attribute name="value"><xsl:value-of select="exslt:node-set($node)/hr:PostalAddress/hr:PostalCode"/></xsl:attribute>
  </input>
  <br/>
  ...
  <select>
   ...
   <xsl:call-template name="countrySelectOptions">
    <xsl:with-param name="code"><xsl:value-of select="exslt:node-set($node)/hr:PostalAddress/hr:CountryCode"/></xsl:with-param>
   </xsl:call-template>
  </select>
 </xsl:template>

 <!-- }}} -->
 <!-- {{{ Template: ELPSelectOptions -->
...
<!-- vim: sw=1 sts=1 ts=1 et fdm=marker nowrap
 -->
